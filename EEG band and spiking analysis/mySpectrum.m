function [bands]= mySpectrum_Mar5(data, fs)
...
while(i <(length(f)+1))
 ...
 if f(i)>4 && f(i)<=8 %% find theta vector 4.1-8 Hz    
    theta(i, 1)= normPxx(i);
    theta(i, 2)= f(i);
 end
 
  if f(i)>8 && f(i)<=13 %% find alpha vector 8.1-13Hz 
    alpha(i, 1)= normPxx(i);
    alpha(i, 2)= f(i);
  end
 ...
 if f(i)>25 && f(i)<=50 %% find gamma vector 25.1-50Hz    
    gamma(i, 1)= normPxx(i);
    gamma(i, 2)= f(i);
 end
 i=i+1;
